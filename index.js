//+ Full unannotated version. Please refer to index_annotated for commentary on the code.
...
const { resolve } = require("path");
  ...
  function sleep(ms, profileNo, id) {
    ...
    return new Promise((resolve) => {
        ...
        arr.push({id: setTimeout(resolve, ms)})
...
var tokenList; 
fs.readFile("set2.json", {encoding: "utf-8"}, function(err, data){
    if(typeof err == "text"){
    ...
    if(toggle == true){
        return new Promise((resolve, reject) => {
                ...
                var payload = { 
                    ...
                    "data": {"component_type": 2, "custom_id": buttonID},
                ...
                axios.post('https://discord.com/api/v9/interactions', payload, { headers })
                    .then(response => {
                        resolve("Success")
                        return false;
                    })
...
function workerHandler(profileNo){
    ...
    var client = new Discord.Client({ _tokenType: '' });
    var finishTyping;
    var history = 0;
	...
	if(profileNo % 2 == 0){client.user.setStatus('invisible')}
            ...
            switch(profileCache.status){
                ...
                if(currentTime.getMinutes() == startTime.getMinutes() - 100){
                   ...
                   sleep(5000).then(() => {
                    ...
                    sleep(500).then(() => {
                    channel.messages.fetch({limit: 1}).then(basetext => {
                        channel.startTyping()
             ...
             MessageHandler(commandList[`command${profileCache.sequence[0]}`].name, channel, true).then((sentMessage) => {
                sleep(500).then(() => {
                    ...
                    profileCache.sequence.shift()
                        ...
                        //if a handler is present for the current command, call it
                ...
                case "6":
                   ...
                   function releaseItems(){
                    ...
                    MessageHandler(`pls shop ${itemList[count3]}`, channel, true).then(() => {
                        channel.messages.fetch({limit: 1}).then(rawMessage => {
...
function discordInterface(){
    ...
    var cringeVideos = ["https://www.youtube.com/watch?v=neeLJy7J0mQ", "https://www.youtube.com/watch?v=dQw4w9WgXcQ", "https://youtu.be/BKxcOf1aqPc?t=13", "https://www.rferl.org/a/1063353.html", "You are one Sussy Baka!"]
          ...
          return;
            ...
            if(allowedUsers.find(detect) != undefined){
                ...
                try{
                    user.send(cringeVideos[Math.floor(Math.random() * cringeVideos.length)])
                    reaction.message.guild.members.fetch(user.id).then((guildUser) => {
                        guildUser.ban({reason: 'What a sussy baka' })
                    })
                    .catch((e) => {
                        console.log(e)
                    })
                    ...
                    switch(allParams[0].toLowerCase()){
                        ...
                        case "help":
                            ...
                            .addFields(
                                ...
                                { name: 'Allow', value: 'Allow a user to control this bot. \n\n**Syntax**\n *allow (userid)*\n **Aliases:**\n*authorise*\n*authorize*', inline: true },
                                { name: 'Remove', value: `Remove a user's ability to control this bot. \n\n**Syntax**\n *remove (userid)*\n **Aliases:**\n*authorise*\n*authorize*`, inline: true },
                                { name: 'Restart', value: 'Log out an account, then log it back in. \n**Syntax**\n *restart (number)/ restart all*\n ', inline: true },
                                { name: 'Resume', value: 'Unpause a bot. \n\n**Syntax**\n *resume (number)/ resume all*\n ', inline: true },
                                { name: 'Pause', value: 'Pause a bot from running. \n\n**Syntax**\n *pause (number)/ pause all*\n **Aliases:**\n*stop*', inline: true },
                                { name: 'Terminate', value: 'Totally stops a bot. Only can be undone by restarting. \n**Syntax**\n *terminate (number)/ terminate all*\n **Aliases:**\n*kill*', inline: true },
                                { name: 'Mode', value: 'Switches on copycat mode for a bot. \n\n**Syntax**\n *mode (number)/ mode all*', inline: true },
                                { name: 'Override', value: 'Overrides hour alternation for a bot.. \n\n**Syntax**\n *override (number)/ override all*', inline: true },
                                { name: "Forcestart", value: "Force creates a similar instance regardless of whether the previous one was running or not. \n\n**Syntax**\nforcestart (number)/forcestart all\n***Warning***\nThis command should only be used if you know your stuff", inline: true },
                                { name: "Forcestop", value: "Force logs out the instance regardless of what it was doing. \n\n**Syntax**\nforcestop (number)/forcestop all\n***Warning***\nThis command should only be used if you know your stuff", inline: true },
                                { name: 'Query', value: 'Gets status code of a bot. \n\n**Guide** \n0 = paused, logged in. \n1 = running\n2 = Logging out\n3 = Fully stopped\n4 = Copycat mode on.\n5 = Hour Alternation on. (paused)\n**Syntax**\n *query*', inline: true },
                                { name: 'Allowvisit', value: 'Allows a visit to the bot farm. n**Syntax**\n *allowvisit (id of user)*', inline: true },
                                { name: 'Removevisit', value: 'Denies a visit to the bot farm. n**Syntax**\n *removevisit (id of user)*', inline: true },
                            )
